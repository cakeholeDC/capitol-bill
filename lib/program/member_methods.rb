...
def prompt_chamber_options(chamber)
	puts ''
	puts "How would you like to search for a #{member_type(chamber)}?"
	puts ''
...
def state_prompt(chamber)
	puts ''
	puts "Which state would you like to find #{member_type(chamber).pluralize} for?"
	puts ''
end
...
def member_menu(member)
	...
	when 2 ## SPONSORED BILLS
		puts ''
		puts "This feature needs to be merged with Bill.rb, please check back soon"
		puts ''
		
		# @TODO uncomment this
		# puts "#{member.full_name}'s Sponsored Bills:"
		# sponsored = Bill.bills_by_lawmaker(member)
		# Bill.narrowing_to_one(sponsored)
	...
	when 4 ## RECENT VOTES
		...
		## @TODO, Uncomment this for Recent Votes
		# votes = member.votes
